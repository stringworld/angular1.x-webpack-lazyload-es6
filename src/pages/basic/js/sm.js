...
function($) {
    ...
    var Picker = function(params) {
        ...
        p.updateValue = function() {
            ...
            if (p.input && p.input.length > 0) {
                var selectedValue = p.params.formatValue ? p.params.formatValue(p, p.value, p.displayValue) : p.value.join(' ');
                $(p.input).val(selectedValue);
                $(p.input).html(selectedValue);
                $(p.input).trigger('change');
                $(p.input).attr('selectedValue', newValue);
                $(p.input).attr('activeIndex', p.activeIndex);
            }
        };
        ...
        p.initPickerCol = function(colElement, updateItems) {
            ...
            col.updateItems = function(activeIndex, translate, transition, valueCallbacks) {
                ...
                col.wrapper.find('.picker-selected').attr('zindex');
                if (p.params.rotateEffect) {
                    col.items.transition(transition);
                }
                var selectedItem = col.items.eq(activeIndex).addClass('picker-selected').transform('');
                p.activeIndex = activeIndex;
                // $(col.items.eq(activeIndex)).click(function() {
                ...
                if (valueCallbacks || typeof valueCallbacks === 'undefined') {
                    ...
                    selectedItem.attr('zindex', activeIndex)
                        // On change callback
